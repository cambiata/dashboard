...
Exercises.random = function(scoreId) {
	...
	(nx3.utils.ScriptScoresXInteraction.instance == null?nx3.utils.ScriptScoresXInteraction.instance = new nx3.utils.ScriptScoresXInteraction():nx3.utils.ScriptScoresXInteraction.instance).onInteractExternal = function(scriptScore,interaction) {
		...
		case 0:
			...
			Exercises.feedback();
			window.document.getElementById("randomFeedback").textContent = Std.string(Exercises.answerNotes.length);
			break;
		default:
		}
	};
	window.document.getElementById("randomFeedback").textContent = "Klicka nu på playknappen för att höra tonföljden";
...
Exercises.feedback = function() {
	...
	if(correct) {
		...
		if(Exercises.correctCount > Exercises.correctIncrease) {
			...
			Exercises.count++;
			js.Lib.alert("Bravo! Nu förlängs övningen till " + Exercises.count + " toner.");
		} else js.Lib.alert("Korrekt!");
		Exercises.clear();
	}
};
Exercises.playRandom = function() {
	...
	this1(function(wav16) {
		haxe.Log.trace("FINISHED nscore1",{ fileName : "Exercises.hx", lineNumber : 172, className : "Exercises", methodName : "playRandom"});
...
nx3.utils.ScriptScoresXInteraction.prototype = {
	...
	,onInteract: function(scriptScore,interaction) {
		...
		if($bind(this,this.onInteractExternal) != null) this.onInteractExternal(scriptScore,interaction);
	}
	,onInteractExternal: function(scriptScore,interaction) {
	}
	,currentActive: null
	,activateScore: function(scriptScore) {
		if(this.currentActive == scriptScore) return;
		(audiotools.sound.Wav16SoundManager.instance == null?audiotools.sound.Wav16SoundManager.instance = new audiotools.sound.Wav16SoundManager():audiotools.sound.Wav16SoundManager.instance).stop();
		haxe.Log.trace("activate",{ fileName : "ScriptScoresXInteraction.hx", lineNumber : 76, className : "nx3.utils.ScriptScoresXInteraction", methodName : "activateScore"});
...
nx3.Constants.FONT_TEXT_X_ADJUST_SYS = -2;
